    ...
    with app.subroute(api_version_string) as app:
        ...
        def directsend(self, request, walletname):
            ...
            try:
                ...
                return make_jmwalletd_response(request, error=f"Missing required parameter: {e.args[0]}")
            except ValueError as e:
                return make_jmwalletd_response(request, error=f"Invalid value: {str(e)}")
            except Exception as e:
                return make_jmwalletd_response(request, error=f"An unexpected error occurred: {str(e)}")
            if not tx:
